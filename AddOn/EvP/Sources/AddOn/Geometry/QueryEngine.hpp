...
public:
    ...
    uint64_t SnapshotId () const { return snapshot->id; }
    size_t   MeshCount ()  const { return snapshot->meshes.size (); }
    size_t   TriangleCount () const { return triToMesh.size (); }

    // Index of a mesh in snapshot->meshes; its guid via MeshGuid.
    const std::string& MeshGuid (size_t idx) const { return snapshot->meshes[idx].guid; }
    int32_t            MeshType (size_t idx) const { return snapshot->meshes[idx].elemType; }
    ...
    PierceResult RaycastAll (const double org[3], const double dir[3],
