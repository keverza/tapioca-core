...
double PointAabbDist (const double p[3], const Aabb& b)
{
    double d2 = 0.0;
    for (int i = 0; i < 3; ++i) {
        const double e = p[i] < b.mn[i] ? b.mn[i] - p[i] : (p[i] > b.mx[i] ? p[i] - b.mx[i] : 0.0);
        d2 += e * e;
    }
    return std::sqrt (d2);
}
...
QueryEngine::QueryEngine (std::shared_ptr<const Snapshot> snap) : snapshot (std::move (snap))
{
...
QueryEngine::RayHit QueryEngine::Raycast (const double org[3], const double dir[3], double maxDist) const
{
    ...
    d[0] /= len;
    d[1] /= len;
    d[2] /= len; // normalise so t is a true distance
...
bool QueryEngine::Occluded (const double org[3], const double dir[3], double tmin, double tmax) const
{
    ...
    d[0] /= len;
    d[1] /= len;
    d[2] /= len; // normalise so tmin/tmax are true distances
    ...
    ray.dir[1] = d[1];
    ray.dir[2] = d[2];
    ray.min_t = (tmin > 0.0) ? tmin : 0.0;
    ray.max_t = (tmax > 0.0) ? tmax : std::numeric_limits<double>::max ();

    // An inverted or empty interval has nothing in it. Returning false rather
    // than clamping keeps a degenerate caller UNSHADOWED, which is visible in a
    // result, instead of shadowed, which reads exactly like a real occluder.
...
QueryEngine::PierceResult QueryEngine::RaycastAll (const double org[3], const double dir[3], double maxDist,
                                                   size_t maxHits) const
{
    ...
    std::sort (out.hits.begin (), out.hits.end (), [] (const PierceHit& a, const PierceHit& b) { return a.t < b.t; });
