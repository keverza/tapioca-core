                    ...
                    bool nudge)
{
    ...
    if (nudge) {
        for (const auto& m : snap.meshes) {
            ...
            if (m.bounds.Valid () && (m.bounds.mn[2] > z || m.bounds.mx[2] < z))
                continue;
            bool tangent = false;
            for (size_t v = 2; v < m.vertices.size (); v += 3) {
                if (std::fabs (m.vertices[v] - z) < kPlaneEps) { tangent = true; break; }
            }
            if (tangent) {
                out.zUsed = z + 1e-6;
                out.nudged = true;
    ...
    for (const auto& m : snap.meshes) {
        ...
        for (size_t t = 0; t < m.triangles.size (); t += 3) {
            const double* p[3] = {
                &m.vertices[m.triangles[t + 0] * 3],
                &m.vertices[m.triangles[t + 1] * 3],
                &m.vertices[m.triangles[t + 2] * 3],
            };
        ...
        es.loops = ChainSegments (segs, nodesXY, z);
        if (!es.loops.empty ())
