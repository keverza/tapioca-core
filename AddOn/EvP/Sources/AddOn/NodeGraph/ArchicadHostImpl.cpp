...
bool ArchicadHostImpl::GetViewCamera (ViewCamera& camera, std::string& error) const
{
    ...
    auto failure = std::make_shared<std::string> ();
    if (!OnHostThread ([read, failure] { geomsrv::ReadArchicadCamera (*read, *failure); }, error))
        return false;
    if (!failure->empty ()) {
        error = *failure;
        return false;
...
bool ArchicadHostImpl::SetViewCamera (const ViewCamera& camera, bool& threeDWindowInFront, std::string& error)
{
    ...
    if (!OnHostThread ([wanted, inFront, failure] { geomsrv::WriteArchicadCamera (*wanted, *inFront, *failure); },
                       error))
        return false;
    if (!failure->empty ()) {
        error = *failure;
