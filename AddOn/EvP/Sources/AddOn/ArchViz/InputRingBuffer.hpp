...
// DG (main thread) -> render thread input.
//
...
// this class carries only what DG is genuinely good at:
//
...
//
// A mutex, not a lock-free ring. It is taken for a few nanoseconds on a DG
// callback and once per frame on the render thread; a hand-rolled lock-free
...
struct InputSnapshot {
    // ---- POLLED by the render thread; NOT pushed by DG --------------------
...
class InputRingBuffer final {
  public:
    ...
    // The viewer is closing, or the panel lost the mouse in a way DG did not
