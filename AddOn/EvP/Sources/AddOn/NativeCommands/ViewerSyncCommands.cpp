...
public:
    ...
    NativeCommandResult ExecuteNative (const GS::ObjectState&, GS::ProcessControl&) const override
    {
        ...
        // How many frames ago that viewport was actually seen at a depth clear.
        os.Add ("gpuSceneAgeFrames", (GS::Int32) capture.sceneCandidateAgeFrames);
        ...
        os.Add ("gpuWindowsCaptured", (GS::Int32) candidates.windowsCaptured);
        os.Add ("gpuWindowsDropped", (GS::Int32) candidates.windowsDropped);
        os.Add ("gpuLargestBoundOffset", (GS::Int32) candidates.largestBoundOffset);
        // What the captured bytes look like, which is what a "no match" needs.
        ...
        os.Add ("gpuPairStraddle", (GS::Int32) pairing.refusedStraddle);
        os.Add ("gpuPairLastAgePasses", (GS::Int32) pairing.lastAgePasses);
        os.Add ("gpuPairLastScenePass", (GS::Int32) pairing.lastScenePass);
        ...
        os.Add ("scenePassOpsAfterBoundary", (GS::Int32) pass.opsAfterBoundary);
        os.Add ("scenePassDrawsAfterBoundary", (GS::Int32) pass.drawsAfterBoundary);
        // ⚠️ IF THE TARGET RETURNS, THE FIRST SWITCH IS NOT THE FINAL BOUNDARY.
        os.Add ("scenePassTargetReturns", (GS::Int32) pass.targetReturns);
        os.Add ("scenePassDrawsAfterReturn", (GS::Int32) pass.drawsAfterReturn);

        // The vertex-stage constant buffers that were ALREADY BOUND when the
        // pass began. A camera constant bound once and never rebound only ever
        // appears here -- see ContextStateTracker.hpp.
        GS::Array<GS::ObjectState> inherited;
        for (int slot = 0; slot < 14; ++slot) {
            if (pass.vsBuffer[slot] == 0)
                continue;
            GS::ObjectState row;
        ...
        os.Add ("scenePassVsBindings", inherited);
        ...
        os.Add ("gpuBlocksAffine", (GS::Int32) candidates.blocksAffine);
        os.Add ("gpuBlocksProjective", (GS::Int32) candidates.blocksProjective);
        os.Add ("gpuFirstProjectiveOffset", (GS::Int32) candidates.firstProjectiveOffset);
