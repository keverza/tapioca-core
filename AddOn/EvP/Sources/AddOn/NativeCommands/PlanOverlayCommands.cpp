  ...
  public:
    ...
    NativeCommandResult ExecuteNative (const GS::ObjectState& params, GS::ProcessControl&) const override
    {
        ...
        if (ACAPI_Window_GetCurrentWindow (&wi) == NoError) {
            ...
            const bool matched = (target == "any") || (target == "plan" && isPlan) || (target == "model3d" && is3D);
        ...
        os.Add ("strictChain", ChainState (po::DescendWindowChain (main, pt, false, false)));
        os.Add ("hitChain", ChainState (po::DescendWindowChain (main, pt, true, false)));
        ...
        os.Add ("hostChain", ChainState (po::DescendWindowChain (main, pt, false, true)));
...
class ProbeOverlayShowCommand : public MainThreadCommand {
  public:
    ...
    NativeCommandResult ExecuteNative (const GS::ObjectState& params, GS::ProcessControl&) const override
    {
        ...
        const RECT rect = { static_cast<LONG> (x), static_cast<LONG> (y), static_cast<LONG> (x + w),
        ...
        HWND overlay = po::Create (parent, canvasForTransform, rect, style, po::Owner::NativeCommand);
        if (!overlay)
            return NativeCommandResult::Failure (GS::UniString::Printf ("CreateWindowEx failed (err %lu)",
  ...
  public:
    ...
    NativeCommandResult ExecuteNative (const GS::ObjectState& params, GS::ProcessControl&) const override
    {
        ...
        if (po::LastPaintTick () != 0) {
            os.Add ("lastPaintMsAgo", static_cast<GS::Int64> (GetTickCount () - po::LastPaintTick ()));
        }
  ...
  public:
    ...
    NativeCommandResult ExecuteNative (const GS::ObjectState& params, GS::ProcessControl&) const override
    {
        ...
        if (mode == "acapi" || mode == "both") {
            ...
            if (err != NoError)
                os.Add ("acapiError",
                        EVP_ACAPI_FAIL ("ACAPI_View_Redraw", err, "forcing a plan redraw for the overlay probe"));
        }

        if (mode == "invalidate" || mode == "both") {
            HWND canvas = ResolveHwndParam (params, "hwnd");
            if (canvas != nullptr) {
                RedrawWindow (canvas, nullptr, nullptr, RDW_INVALIDATE | RDW_ERASE | RDW_UPDATENOW | RDW_NOCHILDREN);
