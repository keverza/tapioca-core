...
class OverlayRuntimeCommand : public MainThreadCommand {
  public:
    ...
    NativeCommandResult ExecuteNative (const GS::ObjectState& params, GS::ProcessControl&) const override
    {
        ...
        // ⚠️ THE ANCHOR IS THE ONLY REGISTRATION
        // MEASUREMENT IN THE TREE. Everything else compares our camera against
        // our own earlier camera; this compares our projection against a point
        // the user can see in Archicad. OverlayController.hpp says what each
        // outcome means.
        if (params.Contains ("anchor")) {
            GS::ObjectState anchor;
            params.Get ("anchor", anchor);
            double ax = 0.0, ay = 0.0, az = 0.0, size = 0.5;
            anchor.Get ("x", ax);
            anchor.Get ("y", ay);
            anchor.Get ("z", az);
            anchor.Get ("size", size);
            av::overlaycontrol::SetOverlayAnchor (ax, ay, az, size);
        }
