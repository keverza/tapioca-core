<script lang="ts">
  ...
  let { status, message, onclick }: { status: NodeResultRecord['status'] | 'pending'; message?: string; onclick: () => void } = $props()
  const noteworthy = $derived(status === 'error' || status === 'blocked')
</script>

<button class:bad={status === 'error'} class:noteworthy type="button" data-menu-toggle title={message || status} {onclick} aria-label={`Node status: ${status}`}>{status}</button>
