<script lang="ts">
  import type { NodeBodyMode } from './types/node'
  ...
  let { id, data, bodyMode, selected, viewerVisible, onbrowse }: { id: string; data: SchemaNodeData; bodyMode: NodeBodyMode; selected: boolean; viewerVisible: boolean; onbrowse: () => void } = $props()
...
{#if bodyMode === 'parameters' || bodyMode === 'parameters+viewer'}
  <NodeControls schema={data.schema} parameters={data.parameters} outputs={data.result?.outputs} />
{:else}
  <section class="ports">
    <div>{#each data.schema.inputs as input}<NodePort port={input} direction="input" />{/each}</div>
    <div>{#each data.schema.outputs as output}<NodePort port={output} direction="output" value={outputText(output.portId)} />{/each}</div>
  </section>
{/if}
...
{:else if data.schema.display === 'text'}
...
{#if viewerVisible && (bodyMode === 'viewer' || bodyMode === 'parameters+viewer')}
